...
import {root,project,finalPath,fullTitle,slides} from './content.mjs';
...
for(const l of [...p.masters.items,...p.layouts.items]) for(const s of l.shapes.items){
 if(s.hasPlaceholderMetadata || /Placeholder|Text Box/.test(s.name??'')){
  ...
  const value=typ==='dateTime'?'28 August 2026':typ==='slideNumber'?'GlobalCommerce':typ==='pic'?'Project illustration':typ==='footer'?'Jawad Ahmad · EduQual Level 6':typ==='title'?'GlobalCommerce':'Enterprise commerce project';
  s.text=value;
 }
}
...
const out=await PresentationFile.exportPptx(p);await out.save(finalPath);
